...
class PdfApp(Page):
    ...
    def __init__(self, *args, **kwargs):
        ...
        label = tkinter.Label(self, text="PDF Ninja Page", fg=TEXT_COLOR, bg=BACKGROUND_COLOR, font=("", 20, "bold"),)
        # label.pack(side="top", fill="both", expand=True)
        label.grid(row=0, column=0, rowspan=4, sticky="nsew")

        self.pdf_t = PdfToolbox()

        buttonframe = tkinter.Frame(self)
        buttonframe.config(bg=SIDEBAR_BACKGROUND_COLOR, width=10, height=10, padx=10)
        container = tkinter.Frame(self)
        container.config(bg=SIDEBAR_BACKGROUND_COLOR, width=10, height=50)
