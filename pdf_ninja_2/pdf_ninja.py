...
class PdfApp(Page):
    ...
    def __init__(self, *args, **kwargs):
        ...
        t_colspan = 1
        t_rowspan = 6
        self.textbox.grid(row=10, column=0, padx=(20, 0), pady=(20, 0), sticky="N", columnspan=t_colspan, rowspan=t_rowspan)
        ...
        self.button_combine_pages.grid(row=1, column=0, padx=(20, 0), pady=(20, 0))
        ...
        self.button_display_pdf_info = customtkinter.CTkButton(
            master=self.tabview_t.tab("commands"),
            text='Display PDF Meta Info',
            command=lambda: self.display_meta_info()
        )
        ...
        self.button_create_pages = customtkinter.CTkButton(
            ...
            command=self.split_pdf_2_pages
        )
        ...
        self.user_entry.grid(row=5, column=0, padx=(20, 0), pady=(20, 0), sticky='N')
        ...
        self.button_merge_pages.grid(row=6, column=0, padx=(20, 0), pady=(20, 0))
        ...
        self.combo_listing_frame = customtkinter.CTkScrollableFrame(
            master=self.tabview_files.tab("combo listing"),
            label_text="Combo PDF",
            width=CMD_FILE_LISTING_WIDTH
        )
        ...
        self.u_tabview_files.add("PDF Combo TST")  # add tab at the end
        self.u_tabview_files.add("TST icon view")  # add tab at the end
        self.u_tabview_files.set("PDF Combo TST")  # set currently visible tab

        self.disp_combo_files_frame = customtkinter.CTkScrollableFrame(
            master=self.u_tabview_files.tab("PDF Combo TST"),
            label_text="COMBO Image Listing",
            width=600
        )
        self.disp_combo_files_frame.grid(row=0, column=1, padx=(20, 0), pady=(20, 0), sticky="nsew")
        self.refresh_combo_listing()

        # split combo selection
        self.combo_split_pages_btn = customtkinter.CTkButton(
            master=self.u_tabview_files.tab("PDF Combo TST"),
            text='Split PDF',
            command=self.split_combo_into_pages
        )
        self.combo_split_pages_btn.grid(row=1, column=0, padx=(20, 0), pady=(20, 0))
        ...
        self.user_flow_combo_listing_frame = customtkinter.CTkScrollableFrame(
            master=self.u_tabview_files.tab("PDF Combo TST"),
            label_text="PDF Combo TST",
            width=200
        )
        self.user_flow_combo_listing_frame.grid(row=0, column=0, padx=(20, 0), pady=(20, 0), sticky="nsew")
        self.user_flow_combo_listing_frame.grid_columnconfigure(0, weight=1)
        self.u_combo_refresh()
    ...
    def u_combo_refresh(self):
        print('in combo_refresh')
        self.user_flow_combo_listing_frame.grid_columnconfigure(0, weight=1)
        self.user_flow_combo_listing_frame_switches = []
        row = 0
        ...
        for f in c_lst:
            # print(f'f:{f}')
            switch_name = customtkinter.CTkSwitch(master=self.user_flow_combo_listing_frame, text=f"{f}")
            switch_name.grid(row=row, column=0, padx=10, pady=(0, 20))
            row += 1
            self.user_flow_combo_listing_frame_switches.append(switch_name)
    ...
    def get_combo_selection(self):
        listing = self.pdf_t.combo_dir()
        # print(f'get_page_selection:listing[]: {listing}')
        selection = []
        for i in range(len(listing)):
            is_selected = self.user_flow_combo_listing_frame_switches[i].get()
    ...
    def split_combo_into_pages(self):
        # print(f'in split_into_pages')
        listing = self.pdf_t.list_combo_dir()
        selection = []

        for i in range(len(self.user_flow_combo_listing_frame_switches)):
            is_selected = self.user_flow_combo_listing_frame_switches[i].get()
            if is_selected == 1:
                selection.append(listing[i])
        ...
        for selected in selection:
            ...
            # print(f'split_execute:selection: {selection}')
            self.pdf_t.split_combo_pdf_into_pages(selected, f'{selected}_split')
            # return True

        self.u_combo_refresh()

    def u_combo_icon_refresh(self):
        u_pages_lst = self.get_combo_listing()
        print(f'u_pages_lst: {u_pages_lst}')
        u_page_listing_frame_switches = []
        ...
        #
        #         # print(f'convert file: {PAGES_DIR}/{in_file}')
        #         img = convert_from_path(f'{COMBO_DIR}/{in_file}')
        #
        #         for i in range(len(img)):
        #             # print(f'in_file: {in_file} :: {img[i]}')
        #
        ...
        for p_img in u_pages_lst:
            ...
            OUT_FILE = f'{PDF_IMG_DIR}/{p_img}'
            IMG_FILE = f'{PDF_IMG_DIR}/{p_img}'
            IN_FILE = f'{COMBO_DIR}/{p_img}'

            # Create a photoimage object of the image in the path

            sz_w = 60
            sz_h = int(sz_w*1.41)
            load_file = COMBO_DIR + '/' + p_img
            ...
            u_switch_name = customtkinter.CTkSwitch(master=self.disp_combo_files_frame, text=IN_FILE)
            u_switch_name.grid(row=u_row_idx, column=u_col_idx, padx=10, pady=(0, 20))
            u_page_listing_frame_switches.append(u_switch_name)
    ...
    def split_pdf_2_pages(self):
        ...
        for selected in selection:
            # selection = f'{selection}'
            # print(f'split_execute:selection: {selection}')
            self.pdf_t.split_pdf_into_pages(PDF_FILES, selected, f'{selected}_split')
            # return True
        self.pages_refresh()
